"""
Populate NJ 2001-2011 general election results from old-format PDFs.

Creates elections and candidacies for:
- Assembly: 2001, 2003, 2005, 2007, 2009, 2011
- Senate: 2001, 2003, 2007 (2011 already in DB)

Also verifies 2011 Senate against parsed data.

Reads parsed data from /tmp/nj_old_format_parsed.json.

Usage:
    python3 scripts/populate_nj_old_format.py --dry-run
    python3 scripts/populate_nj_old_format.py
"""
import sys
import re
import json
...
BATCH_SIZE = 400
INPUT_PATH = '/tmp/nj_old_format_parsed.json'
...
# What we need to populate (chamber, year pairs)
TO_POPULATE = [
    ...
    ('assembly', 2009),
    ('assembly', 2011),
    # 2011 senate already in DB — will verify instead
]
...
def populate_elections(chamber, year, districts, seat_map, existing_candidates, dry_run=False):
    """Populate elections and candidacies for one chamber/year."""
    ...
    print(f'\n  {year} {chamber.title()}: {len(districts)} districts')
    ...
    for d in districts:
        ...
        if chamber == 'assembly':
            for designator in ['A', 'B']:
                seat_key = (office_type, dn, designator)
                seat_id = seat_map.get(seat_key)
                if not seat_id:
                    print(f'    WARNING: No seat for {office_type} D{dn} {designator}')
                    continue
    ...
    for d in districts:
        if chamber == 'assembly':
            winners = sorted([c for c in d['candidates'] if c['winner']], key=lambda c: -c['votes'])
